...
// on Linux when sharing with Windows or macOS.
...
// On Linux, hidden files start with ".".
// We also exclude known cross-platform metadata files.
func isHiddenUnixEntry(entry fs.DirEntry) bool {
	name := entry.Name()

	// Exclude known metadata files (can appear on Linux when sharing with Windows/macOS)
