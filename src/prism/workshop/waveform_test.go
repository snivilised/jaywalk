...
var _ = Describe("WaveformVisualiser", func() {
	var v *workshop.WaveformVisualiser

	BeforeEach(func() {
		v = &workshop.WaveformVisualiser{Width: 40}
	})

	It("returns the name 'waveform'", func() {
		Expect(v.Name()).To(Equal("waveform"))
	})
	...
	DescribeTable("contains height-varying chars",
		func(entry stepsEntry) {
			result := v.Render(
				makeSteps(entry.Count),
				entry.Curve,
