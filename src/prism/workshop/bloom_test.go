...
var _ = Describe("BloomVisualiser", func() {
	var v *workshop.BloomVisualiser

	BeforeEach(func() {
		v = &workshop.BloomVisualiser{Width: 40}
	})

	It("returns the name 'bloom'", func() {
		Expect(v.Name()).To(Equal("bloom"))
	})
	...
	DescribeTable("contains bloom characters",
		func(entry stepsEntry) {
			result := v.Render(
				makeSteps(entry.Count),
				entry.Curve,
