...
func (v *WaveformVisualiser) Render(
	...
	for col := 0; col < v.Width; col++ {
		...
		writeColouredChar(&b, steps, stepIdx, heightChars[idx])
	}
	...
	for col := 0; col < v.Width; col++ {
		...
		writeColouredChar(&b, steps, stepIdx, "█")
	}

	return b.String()
}

func writeColouredChar(b *strings.Builder, steps []contract.Color, stepIdx int, ch string) {
	c := steps[stepIdx]
